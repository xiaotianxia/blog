...
## promise 异常捕获
Q1:在多个Promise调用链中如何在某个then中捕捉到错误，但是不停止then语句的执行？
若链路中也对错误进行了捕获，则后续的函数可能会继续执行
```js
...
在promise调用链中，某个 then 里面发生异常后，就会跳过 then 方法，直接执行 catch。

Q2: 如果在某个then中捕捉到错误，则立即停止then语句的执行
...
为了防止有漏掉的 Promise 异常，建议在全局增加一个对 unhandledrejection 的监听，用来全局监听Uncaught Promise Error。使用方式：
